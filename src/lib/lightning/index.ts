...
export const sendPaymentV2Sync = (
  ...
  const options: routerrpc.ISendPaymentRequest = {
    ...
    feeLimitSat: Long.fromValue(
      Math.max(10, (payAmount?.toNumber() || 0) * maxFeeRatio),
    ),
    cltvLimit: 0,
    outgoingChanId,
  };
...
export const sendKeysendPaymentV2 = (
  ...
  const options: routerrpc.ISendPaymentRequest = {
    ...
    maxParts: 2,
    feeLimitSat: Long.fromValue(
      Math.max(10, (sat?.toNumber() || 0) * maxFeeRatio),
    ),
    cltvLimit: 0,
  };
  if (tlvRecordNameStr && tlvRecordNameStr.length > 0) {
    options.destCustomRecords![TLV_RECORD_NAME] =
      unicodeStringToUint8Array(tlvRecordNameStr);
  }
 ...
 * @throws
 */
export const trackPaymentV2Sync = async (
  paymentHash: string,
): Promise<lnrpc.Payment> => {
 ...
 * @throws
 */
export const queryMissionControl =
  async (): Promise<routerrpc.QueryMissionControlResponse> => {
    const response = await sendCommand<
