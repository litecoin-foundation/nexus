...
export const listChannels = () => async (dispatch) => {
  const {value} = await lnd.listChannels();
  const {remotePubkey, capacity, localBalance, remoteBalance, active} = value;
  dispatch({
    type: LIST_CHANNELS,
    remotePubkey,
    capacity,
...
export const listPeers = () => async (dispatch) => {
  const response = await LndInstance.sendCommand('ListPeers');
