...
import {View, TouchableOpacity, Text, StyleSheet, Platform, Dimensions, Image} from 'react-native';
...
const ChooseWalletButton: React.FC<Props> = props => {
  ...
  const styles = StyleSheet.create({
    ...
    boxArrow: {
      ...
      alignItems: 'center',
      justifyContent: 'center',
      transform: isModalOpened ? 'rotate(90deg)' : 'rotate(-90deg)',
      objectFit: 'contain',
      marginTop: 2,
      marginLeft: 10,
    },
    boxLeftCurve: {
      position: 'absolute',
      bottom: 0,
      left: Dimensions.get('screen').height * 0.02 * -1,
    ...
    boxRightCurve: {
      position: 'absolute',
      bottom: 0,
      right: Dimensions.get('screen').height * 0.02 * -1,
      height: Dimensions.get('screen').height * 0.02,
      width: Dimensions.get('screen').height * 0.02,
    },
    disabled: {
  ...
  return (
    <TouchableOpacity
      ...
      <View style={[styles.buttonBox, customStyles]}>
        ...
        <Image style={styles.boxArrow} source={require('../../assets/images/back-icon.png')} />
        {isBottomCurvesEnabled ? (
          <>
            <Svg
